...
function abrirDetalhes(titulo, sinopse, classificacao, duracao, imgUrl, posterCatalogoUrl) {
    ...
    // Atualiza o texto do botão Minha Lista
    const btnLista = document.getElementById('btn-lista-toggle');
    if (minhaListaFilmes.some(f => f.titulo === filmeAtualId)) {
...
function voltarParaHome() {
    ...
    btnVoltarDaLista.classList.add('oculto'); // Esconde o botão voltar quando já está na home
...
function abrirMinhaLista() {
    ...
    tituloSecao.innerText = "Minha Lista";
    containerFiltro.style.display = "none"; 
    btnVoltarDaLista.classList.remove('oculto'); // Mostra o botão voltar na tela "Minha Lista"
...
function alternarMinhaLista() {
    const btnLista = document.getElementById('btn-lista-toggle');
    const index = minhaListaFilmes.findIndex(f => f.titulo === filmeAtualId);

    if (index > -1) {
        minhaListaFilmes.splice(index, 1);
        btnLista.innerText = "+ Minha Lista";
    } else {
        minhaListaFilmes.push(filmeAtualDados);
        btnLista.innerText = "✓ Na Minha Lista";
    }
}
...
function renderizarMinhaLista() {
    ...
    if (minhaListaFilmes.length === 0) {
        gradeMinhaLista.innerHTML = "<p style='color: #808080; padding-left: 5px;'>Nenhum filme salvo na sua lista ainda.</p>";
        return;
    }
